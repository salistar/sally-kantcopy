...
export default function LobbyScreen() {
  ...
  const handleShare = async (platform: string) => {
    if (!room) return;
    const msg = `Rejoins-moi sur SallyCards Concentration ! Code: ${room.code}\n${room.shareUrl}`;
    log.screen('share', platform);
    try {
      await Share.share({ message: msg, url: room.shareUrl, title: `SallyCards Concentration — ${room.code}` });
