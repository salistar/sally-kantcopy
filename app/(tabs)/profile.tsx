 ...
 * @description Profil Concentration avec vraies valeurs (stats/coins/achievements/
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const msg = e?.message ?? e?.toString?.() ?? 'unknown';
        const status = e?.status ?? '?';
        if (status === 404) log.explain('pas encore de rang concentration pour ce joueur');
