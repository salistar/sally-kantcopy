/**
 * @file (tabs)/index.tsx
 * @description Home Concentration. 4 actions principales : Créer, Rejoindre, vs Bot
...
export default function HomeScreen() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      setUser(u);
      log.bin('GET /challenges/daily/concentration');
      const ch = await api.getDailyChallenge('kantcopy');
      if (ch) {
        log.bout('200 /challenges/daily/concentration', { title: ch.title, reward: ch.rewardCoins });
        setChallenge(ch);
      }
  ...
  // SOLO Memory : ouvre le sélecteur des 13 variantes (1-N joueurs).
  const handlePlay = () => {
  ...
  const handleChallenge = async () => {
    log.bin('POST /challenges/daily/concentration/matchmake');
    try {
      const room = await api.joinDailyChallenge('kantcopy');
  ...
  return (
    <View style={styles.root}>
      ...
      >
        ...
        {/* SOLO Memory — CTA principale → sélecteur 13 variantes */}
        <TouchableOpacity style={{ borderRadius: 18, overflow: 'hidden', marginTop: 12 }} onPress={handlePlay} activeOpacity={0.88}>
          <LinearGradient colors={palette.accentGradient} start={{x:0,y:0}} end={{x:1,y:1}} style={{ flexDirection: 'row', alignItems: 'center', gap: 14, paddingHorizontal: 18, paddingVertical: 20 }}>
            ...
            <View style={{ flex: 1 }}>
              <Text style={{ color: '#fff', fontSize: 18, fontFamily: 'Inter-Black', letterSpacing: 0.5 }}>{t('home.playCta')}</Text>
              <Text style={{ color: 'rgba(255,255,255,0.9)', fontSize: 12, fontFamily: 'Inter-SemiBold', marginTop: 2 }}>{t('home.playCtaSub')}</Text>
            </View>
            <View style={{ backgroundColor: 'rgba(255,255,255,0.2)', paddingHorizontal: 10, paddingVertical: 4, borderRadius: 999 }}>
              <Text style={{ color: '#fff', fontSize: 10, fontFamily: 'Inter-Black', letterSpacing: 1 }}>{t('home.soloBadge')}</Text>
            </View>
          </LinearGradient>
        </TouchableOpacity>
        ...
        <View style={styles.actionRow}>
          <TouchableOpacity style={styles.actionCard} onPress={() => goVariant('classic-4x4')} activeOpacity={0.85}>
            <LinearGradient colors={['#2563EB', '#06B6D4']} start={{x:0,y:0}} end={{x:1,y:1}} style={styles.actionGradient}>
              <Text style={{ fontSize: 30 }}>🟥</Text>
              <Text style={styles.actionTitle}>{t('home.shortClassic')}</Text>
              <Text style={styles.actionSub}>{t('home.shortClassicSub')}</Text>
            </LinearGradient>
          </TouchableOpacity>
          <TouchableOpacity style={styles.actionCard} onPress={() => goVariant('kant-voleur')} activeOpacity={0.85}>
            <LinearGradient colors={['#7C3AED', '#EC4899']} start={{x:0,y:0}} end={{x:1,y:1}} style={styles.actionGradient}>
              <Text style={{ fontSize: 30 }}>🤹</Text>
              <Text style={styles.actionTitle}>{t('home.shortVoleur')}</Text>
              <Text style={styles.actionSub}>{t('home.shortVoleurSub')}</Text>
            </LinearGradient>
          </TouchableOpacity>
          <TouchableOpacity style={styles.actionCard} onPress={() => goVariant('vs-ai')} activeOpacity={0.85}>
            <LinearGradient colors={['#10B981', '#059669']} start={{x:0,y:0}} end={{x:1,y:1}} style={styles.actionGradient}>
              <Text style={{ fontSize: 30 }}>🤖</Text>
              <Text style={styles.actionTitle}>{t('home.shortAi')}</Text>
              <Text style={styles.actionSub}>{t('home.shortAiSub')}</Text>
            </LinearGradient>
          </TouchableOpacity>
        </View>

        {/* Multi (Create/Join) — pour Coopératif et Vol */}
        <View style={styles.actionRow}>
